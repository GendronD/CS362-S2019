...
// Minion - no mistakes added
int minionCard(int i, int j, int handPos, struct gameState *state, int currentPlayer, int choice1, int choice2)
{
  //+1 action
  state->numActions++;
...
// Ambassador - mistake added
int ambassadorCard(int i, int j, int choice1, int choice2, int handPos, struct gameState *state, int currentPlayer)
{
  ...
  //****************************************************************************
  //if (choice1 == handPos) //correct
  if (choice1 != handPos) // bug
  //****************************************************************************
  {
    return -1;
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
    return adventurerCard(drawntreasure, currentPlayer, state, cardDrawn, temphand, z);
    /*while (drawntreasure < 2)
    {
      if (state->deckCount[currentPlayer] < 1)
      { //if the deck is empty we need to shuffle discard and add to deck
  ...
  case minion:
    return minionCard(i, j, handPos, state, currentPlayer, choice1, choice2);
    /*
    //+1 action
    state->numActions++;
  ...
  case ambassador:
    return ambassadorCard(i, j, choice1, choice2, handPos, state, currentPlayer);
    /*j = 0; //used to check if player has enough cards to discard
  ...
  case outpost:
    return outpostCard(handPos, currentPlayer, state);
    /*
    //set outpost flag
    state->outpostPlayed++;
